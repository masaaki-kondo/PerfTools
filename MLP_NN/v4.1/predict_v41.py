"""v4.1 cross-GPU kernel performance estimator -- CLI (self-contained).

v4.1 = the released NO-ET pure NN (no Execution Time as a predictive input; no
...
EXAMPLES
  python MLP_NN/v40/predict_v41.py \
...
import v40_core                            # v4.1 no-ET inference
...
PROVENANCE = [
    "# v4.1 cross-GPU kernel estimator (no-ET pure NN) -- output",
    ...
    "# MODEL: v4.1 -- Execution Time is NOT a predictive input; inference is a plain NN forward pass.",
    "# OUTPUTS: ExecTime/Mem%/Occupancy/brk_* = v4.1 predictions; t_*_ns + efficiency_eta = roofline.",
]
...
def main():
    ap = argparse.ArgumentParser(
        description="v4.1 cross-GPU kernel estimator (no-ET pure NN). Two input modes: "
                    "(1) full input via CLI options, or (2) --csv FILE [--row N].")
    ap.add_argument("--artifact", default=f"{HERE}/v41_artifact")
    ...
    if args.dump_records:
        n = core.dump_records(args.dump_records, "v4.1", samples, out_rows,
                              v40_core._features, core.DERIVED_NAMES_V40, args.no_comments)
        sys.stderr.write(f"records  : {n} rows -> {args.dump_records}\n")

    log = [f"v4.1 cross-GPU estimator (no-ET pure NN)  run @ {datetime.datetime.now().isoformat(timespec='seconds')}",
           f"model    : v4.1  ({meta['seeds']}-seed ensemble)", "specs    : read from the input (SRC/TGT columns/options)",
